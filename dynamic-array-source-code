...
public class Array <T> implements iterable <T>{
 private T [] arr;
 private int len = 0;  //Array length by user
 ...
 public T removeAt(int rm_index) {
  ...
  for(int i = 0, j = 0;i < len; i++ , j++)
    if(i == rm_index) j--; //skip overrm_index by fixing j temporarily
 ...
 @Override public java.util.Iterator <T> iterator() {
  returnn new java.util.Itereator <T> (){
    int index = 0;
